...
namespace VaynePlus.Utils
{
    class PositionalHelper
    {
        ...
        private static Team StrongerTeam
        {
            get
            {
                var enemyCs = HeroManager.Enemies.FindAll(m => m.Distance(ObjectManager.Player) <= Range && m.IsValidTarget(Range)).Aggregate(0f, (current, enemy) => current + (enemy.MinionsKilled + enemy.NeutralMinionsKilled));
                var allyCs = HeroManager.Allies.FindAll(m => m.Distance(ObjectManager.Player) <= Range && m.IsValidTarget(Range)).Aggregate(0f, (current, ally) => current + (ally.MinionsKilled + ally.NeutralMinionsKilled));
                var allyKills = HeroManager.Allies.FindAll(m => m.Distance(ObjectManager.Player) <= Range && m.IsValidTarget(Range)).Aggregate(0f, (current, ally) => current + (ally.ChampionsKilled));
                var enemyKills = HeroManager.Enemies.FindAll(m => m.Distance(ObjectManager.Player) <= Range && m.IsValidTarget(Range)).Aggregate(0f, (current, enemy) => current + (enemy.ChampionsKilled));
                return allyCs * 17.5f + allyKills * 300f >= enemyCs * 17.5f + enemyKills * 300f ? Team.Ally : Team.Enemy;
                // return Team.Ally;
            }
        }

        private static IEnumerable<AIHeroClient> AlliesClose
        {
            get
            {
                return
                    HeroManager.Allies.FindAll(
                        m =>
                            m.Distance(ObjectManager.Player) <= Range && m.IsValidTarget(Range) &&
